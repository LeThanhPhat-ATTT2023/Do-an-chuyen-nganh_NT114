...
def load_v3_artifact(
    ...
    """Load a Smart-BOTH Hybrid graph artifact (``artifact_version='v3'``).

    5-node-type schema: flow, packet, host, technique, tactic. Packet->technique
    edges are typed by attack family (injection / command_exec / file_upload /
    recon / c2_beacon) — five typed ``packet -> evidence_{family} -> technique``
    relations so HGT can learn per-family attention.

    Schema highlights:
      * ``host`` node type — 4-d aggregate features from flow src/dst IPs
      * ``flow -> burst_neighbor -> flow`` homophily edges
      * ``flow -> from_host -> host`` / ``flow -> to_host -> host`` edges
      * ``technique -> has_subtechnique -> technique`` hierarchy edges
    ...
    ``packet_dtype='preserve'`` keeps the on-disk (float16) precision; the
    default upcasts packet_x to float32.
    """
    graph_npz = Path(graph_npz)
    if graph_meta_json is None:
        graph_meta_json = graph_npz.with_suffix(".meta.json")
